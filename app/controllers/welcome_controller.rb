class WelcomeController < ApplicationController
 ...
 def get_text_string
   add_paragraphs
   return_highlighted_text @text_string.split(), $highlights
   @text_string
 end
 ...
 #Setters
 def set_text_string
    @text_string, @arr = $content, $highlights;
 end
 ...
 #Add HTML Paragraphs
 private def add_paragraphs
    #Step 1: Replace double new lines with <br>
    @text_string = $content.gsub!(/\r?\n/, "<br>");

    #Step 2: Replace <br> with </p><p>
    @text_string = @text_string.gsub!(/<br>\s*<br>/, "</p><p>")

    #Step 3: Add the first and last <p>
    @text_string = "<p><div style='display:inline'>"+@text_string+"</p>"

    return @text_string;
 end

 private def return_highlighted_text (arr_val,hash_arr)
   colors = ["red","blue","green"];

   #Loop through the hash array to pick the values
   hash_arr.each do |obj|
      start_val = obj[:start]
      end_val = obj[:end]
      comment = obj[:comment]
      #index = hash_arr.find_index(obj)
      index_color = colors[hash_arr.find_index(obj)]
      
      #insert a background color
      arr_val.insert(start_val,"</div><div style='display:inline' title='#{comment}'><span style='background-color:#{index_color}'>");
      arr_val.insert(end_val,"</span></div>")
   end
   @text_string = arr_val.join(" ");
   return @text_string;
 end
end
